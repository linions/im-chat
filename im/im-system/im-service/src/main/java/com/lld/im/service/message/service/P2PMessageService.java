...
public class P2PMessageService {
    ...
    public void process(MessageContent messageContent){
        ...
        if(appConfig.isSendMessageBeforeCallback()){
            responseVO = callbackService.beforeCallback(messageContent.getAppId(), Constants.CallbackCommand.SendMessageBefore, JSONObject.toJSONString(messageContent));
        }
        ...
        writeUserSeq.writeUserSeq(messageContent.getAppId(),messageContent.getFromId(),Constants.SeqConstants.Message + ":" + messageContent.getToId(),seq);
        //前置校验
        //这个用户是否被禁言 是否被禁用
        //发送方和接收方是否是好友
            threadPoolExecutor.execute(() ->{
...
//                messageStoreService.storeOfflineMessage(offlineMessageContent);
    ...
    public void receiverAck(MessageContent messageContent){
        ...
        messageProducer.sendToUser(messageContent.getFromId(),MessageCommand.MSG_RECEIVE_ACK, pack,new ClientInfo(messageContent.getAppId(),messageContent.getClientType(),messageContent.getImei()));
    }
    ...
    public SendMessageResp send(SendMessageReq req) {
        ...
        long seq = redisSeq.doGetSeq(req.getAppId() + ":" + Constants.SeqConstants.Message+ ":" + ConversationIdGenerate.generateP2PId(req.getFromId(),req.getToId()));
