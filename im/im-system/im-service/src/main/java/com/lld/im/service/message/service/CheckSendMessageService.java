...
public class CheckSendMessageService {
    ...
    public ResponseVO checkSenderForbidAndMute(String fromId,Integer appId){
        ...
        ResponseVO<GetSingleUserInfoResp> singleUserInfo = imUserService.getSingleUserInfo(fromId, appId);
        if(!singleUserInfo.isOk()){
            return singleUserInfo;
        }

        GetSingleUserInfoResp user = singleUserInfo.getData();
    ...
    public ResponseVO checkSenderForbidAndMute(String fromId,String groupId,Integer appId){
        ResponseVO<GetSingleUserInfoResp> singleUserInfo = imUserService.getSingleUserInfo(fromId, appId);
        if(!singleUserInfo.isOk()){
            return singleUserInfo;
        }
        ImGroupMemberEntity member = imGroupMemberService.getGroupMemberByMemberId(groupId, appId, fromId);

        if(member.getMute() == UserForbiddenFlagEnum.FORBIBBEN.getCode()){
            return ResponseVO.errorResponse(MessageErrorCode.FROMER_IS_MUTE);
        }
        return ResponseVO.successResponse();
    }
    ...
    public ResponseVO checkGroupMessage(String fromId,String groupId,Integer appId){
        ...
        ResponseVO responseVO = checkSenderForbidAndMute(fromId,groupId, appId);
        if(!responseVO.isOk()){
            return responseVO;
        }
        ...
        Date now = new Date();
        if(data.getMute() == GroupMuteTypeEnum.MUTE.getCode()  && data.getSpeakDate() != null && data.getSpeakDate().before(now) ){
