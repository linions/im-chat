...
export default class WebToolkit {
  ...
  // 获取客户端设备信息
  public static getDeviceInfo(type:Number):any {
    let deviceInfo = null
    

    if(type == 1){
    /*IFTRUE_WEBAPP*/
    deviceInfo = WebToolkit.getBrowserInfo();
    /*FITRUE_WEBAPP*/
    }
    ...
    if(type == 2){
      /*IFTRUE_WXAPP*/
          // 小程序的情况需要把pv.sohu.com域名加入白名单中
          deviceInfo = WebToolkit.getWxappInfo();
          /*FITRUE_WXAPP*/
      }

    if(type == 3){
     /*IFTRUE_RNAPP*/
    deviceInfo = WebToolkit.getReactNativeInfo();
    /*FITRUE_RNAPP*/
    }

    if(type == 4){
      /*IFTRUE_UNIAPP*/
    deviceInfo = WebToolkit.getUniAppInfo();
    /*FITRUE_UNIAPP*/
    }
    return deviceInfo;
  }
