<template>
    ...
    <el-col class="component-upload-image" style="display: flex;justify-content: center;">
        <el-upload
        multiple
    ...
    <el-col style="display: flex;justify-content: center;margin: 5px 0 10px 0;">
      ...
      <el-upload
        ...
        :disabled="isDisabled"
        >
        <el-button  style="background-color: #8a98c9;" type="primary" round>更换头像</el-button>
      </el-upload>
  ...
  import {uninstallUserLogo} from '../../api/user/user'
...
console.log("userStore = ",userStore)
  ...
  const uploadImgUrl = ref("/v1/user/uploadLogo/" + userStore.userInfo.userId +"/"+userStore.userInfo.appId) // 上传的图片服务器地址
  const fileList = ref([])
  ...
  function handleRemove() {
    console.log("fileList.value = " ,fileList.value)
    uninstallUserLogo(fileList.value[0].name)
    fileList.value = []
    // emit('update:modelValue', listToString(uploadFileList.value))
  }
  ...
  function handleUploadSuccess(res) {
    if (res.code != 200) {
      ...
      if (uploadList.value.length === number.value) {
        uploadFileList.value = uploadList.value
      //   fileList.value = uploadFileList.value
        uploadList.value = []
        number.value = 0
        emit('modelValue', false)
        moadl.msgSuccess("头像修改成功")
        location.reload()
      }
      moadl.closeLoading()
    }
